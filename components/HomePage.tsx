...
import { currency_selector_state, setDefaultCurrencyValue } from '../store/slices/general_slices/multi-currency-slice';
...
const HomePage = ({ default_currency_value, multi_lingual_values }: any) => {
  ...
  const [displayTagDataFromReducer, setDisplayTagDataFromReducer] = useState<any>([]);
  ...
  useEffect(() => {
    if (allTagsData?.length > 0) {
      allTagsData?.map((tag: any, i: any) => tagListArray?.push(tag));
  ...
  return (
    <>
      <HomeBanner />
      <HomeTopCategories homeTopCategories={homeTopCategories} isLoading={isLoading} selectedCurrencyVal={selectedCurrencyVal} />
